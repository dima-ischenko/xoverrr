...
class TestUtils:
    ...
    @pytest.mark.parametrize(
        'df1_ids, df2_ids, expected_score',
        [
    ...
    def test_compare_dataframes_parametrized(self, df1_ids, df2_ids, expected_score):
        ...
        df1 = pd.DataFrame({'id': df1_ids, 'value': ['A', 'B', 'C']})

        df2 = pd.DataFrame(
            {'id': df2_ids, 'value': ['A', 'B', 'C' if df1_ids == df2_ids else 'X']}
        )
    ...
    def test_cross_fill_missing_dates(self):
        ...
        df1 = pd.DataFrame(
            {'dt': pd.to_datetime(['2023-01-01', '2023-01-02']), 'cnt': [10, 20]}
        )

        df2 = pd.DataFrame(
            {'dt': pd.to_datetime(['2023-01-02', '2023-01-03']), 'cnt': [15, 25]}
        )
    ...
    def test_performance_medium_dataframe(self):
        ...
        change_indices = np.random.choice(
            np.arange(1, n_records), size=k, replace=False
        )
    ...
    def test_compound_primary_key(self):
        ...
        df2 = pd.DataFrame(
            {'id1': [1, 2, 2], 'id2': ['a', 'a', 'b'], 'value': [10, 30, 40]}
        )
    ...
    def test_duplicate_primary_keys_in_source(self):
        ...
        df2 = pd.DataFrame({'pk': [1, 2, 3, 4], 'value': ['A', 'C', 'D', 'E']})
    ...
    def test_compound_primary_key_all_different(self):
        ...
        df1 = pd.DataFrame(
            {'key1': [1, 2, 3], 'key2': ['X', 'Y', 'Z'], 'value': [10, 20, 30]}
        )

        df2 = pd.DataFrame(
            {'key1': [4, 5, 6], 'key2': ['A', 'B', 'C'], 'value': [40, 50, 60]}
        )
    ...
    def test_edge_case_all_different(self):
        ...
        df2 = pd.DataFrame({'id': [4, 5, 6], 'value': ['x', 'y', 'z']})
    ...
    def test_compound_primary_key_with_duplicates(self):
        ...
        df2 = pd.DataFrame(
            {'id1': [1, 2, 3], 'id2': ['a', 'a', 'c'], 'value': [11, 30, 40]}
        )
    ...
    def test_duplicate_primary_keys_in_target(self):
        ...
        df1 = pd.DataFrame({'pk': [1, 2, 3, 4], 'value': ['A', 'C', 'D', 'E']})
