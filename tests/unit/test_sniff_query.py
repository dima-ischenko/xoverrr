...
from xoverrr.utils import (
    ...
    resolve_sniff_query_issue_column,
)
...
class TestSniffQueryUtils:
    ...
    def test_resolve_sniff_query_issue_column_row_level(self):
        assert (
            resolve_sniff_query_issue_column(['id', XSNIFF_ISSUE_COLUMN])
            == XSNIFF_ISSUE_COLUMN
        )

    def test_resolve_sniff_query_issue_column_scalar(self):
        assert (
            resolve_sniff_query_issue_column([XSNIFF_ISSUE_COLUMN])
            == XSNIFF_ISSUE_COLUMN
        )

    def test_resolve_sniff_query_issue_column_rejects_unknown_shape(self):
        with pytest.raises(ValueError, match=XSNIFF_ISSUE_COLUMN):
            resolve_sniff_query_issue_column(['id', 'name'])
...
class TestSniffQuery:
    ...
    def test_sniff_query_row_level(self, monkeypatch):
        source_df = pd.DataFrame(
            {
                'order_id': [1, 2, 3],
                XSNIFF_ISSUE_COLUMN: [FLAG_VALUE_NO, FLAG_VALUE_NO, FLAG_VALUE_YES],
            }
        )
        metadata = pd.DataFrame(
            {'column_name': ['order_id', XSNIFF_ISSUE_COLUMN]}
        )
        comparator = self._build_comparator(monkeypatch, source_df, metadata)

        status, report, stats, details = comparator.sniff_query(
            source_query='SELECT order_id, xsniff_issue FROM orders',
    ...
    def test_sniff_query_pass_fail_scalar(self, monkeypatch):
        source_df = pd.DataFrame({XSNIFF_ISSUE_COLUMN: [FLAG_VALUE_NO]})
        metadata = pd.DataFrame({'column_name': [XSNIFF_ISSUE_COLUMN]})
        comparator = self._build_comparator(monkeypatch, source_df, metadata)

        status, report, stats, details = comparator.sniff_query(
            source_query="SELECT 'n' AS xsniff_issue",
    ...
    def test_sniff_query_failure(self, monkeypatch):
        ...
        status, report, stats, details = comparator.sniff_query(
            source_query="SELECT 'y' AS xsniff_issue",
            tolerance_percentage=0.0,
        )
