"""ClickHouse count mismatch: source has an extra day."""
...
class TestClickHouseCountMismatch:
    @pytest.fixture(autouse=True)
    def setup_mismatch_data(self, clickhouse_engine, table_helper):
        source_table = 'test_ch_counts_mismatch_src'
        target_table = 'test_ch_counts_mismatch_trg'
    ...
    def test_total_counts_mismatch(self, clickhouse_engine):
        checker = DataQualityChecker(
            source_engine=clickhouse_engine,
            target_engine=clickhouse_engine,
            timezone='UTC',
        )
        source_ref = DataReference('test_ch_counts_mismatch_src', 'test')
        target_ref = DataReference('test_ch_counts_mismatch_trg', 'test')
    ...
    def test_counts_group_by_date_mismatch(self, clickhouse_engine):
        checker = DataQualityChecker(
            source_engine=clickhouse_engine,
            target_engine=clickhouse_engine,
            timezone='UTC',
