"""
Year-range custom-query chunking integration tests for Oracle.
"""
...
class TestOracleCustomQueryYearlyChunking:
    @pytest.fixture(autouse=True)
    def setup_yearly_data(self, oracle_engine, table_helper):
        source_table = 'test_ora_custom_query_chunking_yearly'
        target_table = 'test_ora_custom_query_chunking_yearly_target'
        ...
        table_helper.create_table(
            ...
            insert_sql=f"""
                ...
                SELECT LEVEL, 'name-' || LEVEL, DATE '2024-01-01' + (LEVEL - 1)
                FROM dual
    ...
    def test_custom_query_chunking_yearly_positive(self, oracle_engine):
        comparator = DataQualityComparator(oracle_engine, oracle_engine, timezone='UTC')
        query = """
            SELECT id, name, created_at
            FROM test.test_ora_custom_query_chunking_yearly
            WHERE created_at >= to_date(:start_date, 'YYYY-MM-DD')
              AND created_at <= to_date(:end_date, 'YYYY-MM-DD')
        """
    ...
    def test_custom_query_chunking_yearly_negative(self, oracle_engine):
        comparator = DataQualityComparator(oracle_engine, oracle_engine, timezone='UTC')
        source_query = """
            SELECT id, name, created_at
            FROM test.test_ora_custom_query_chunking_yearly
        ...
        target_query = """
            SELECT id, name, created_at
            FROM test.test_ora_custom_query_chunking_yearly_target
            WHERE created_at >= to_date(:start_date, 'YYYY-MM-DD')
              AND created_at <= to_date(:end_date, 'YYYY-MM-DD')
        """
        params = {'start_date': '2024-01-01', 'end_date': '2024-12-31'}
