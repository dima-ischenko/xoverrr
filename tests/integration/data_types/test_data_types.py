# ci/tests/integration/test_data_types.py
import pytest
import sys
import os
...
class TestPostgresSelfComparison:
    ...
    def setup_postgres_view(self, postgres_engine):
        ...
        with postgres_engine.begin() as conn:
            # Create a view
            conn.execute(text("DROP VIEW IF EXISTS test_customers_view CASCADE"))
            conn.execute(text("""
                CREATE VIEW test_customers_view AS
    ...
    def test_postgres_table_vs_view(self, postgres_engine):
        ...
        status, report, stats, details = comparator.compare_sample(
            source_table=DataReference("customers", "test"),  # таблица
            target_table=DataReference("test_customers_view", "test"),  # вьюха
            date_column="created_at",
            date_range=("2024-01-01", "2024-01-03"),
            tolerance_percentage=0.0,
        )
